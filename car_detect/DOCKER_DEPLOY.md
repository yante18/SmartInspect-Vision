...
```
/opt/car-detect/
├── main.py              ← 项目代码（挂载到容器 /app）
├── models/              ← 模型模块（挂载到容器 /app/models）
├── static/              ← 静态文件（挂载到容器 /app/static）
│   └── upload/          ← 上传文件（单独持久化卷）
├── data/                ← 数据库目录（单独持久化卷）
├── logs/                ← 日志目录（单独持久化卷）
├── docker-compose.yml   ← Docker编排配置
└── Dockerfile           ← 镜像构建文件（仅安装依赖）
```
...
- 代码修改后只需重启容器即可生效，无需重新构建镜像
- 数据卷（upload、data、logs）独立于代码卷，保证数据持久化
