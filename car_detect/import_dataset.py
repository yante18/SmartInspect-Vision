...
def find_dataset_root(source):
    ...
    # Check if current directory has images/labels structure (or label)
    if (source / "images").exists() or (source / "labels").exists() or (source / "label").exists():
        return source
    ...
    for item in source.iterdir():
        if item.is_dir():
            ...
            if (item / "images").exists() or (item / "labels").exists() or (item / "label").exists():
                return item
...
def import_dataset(source_path, target_dir=None):
    ...
    # Case 1: Standard YOLO structure (images + labels)
    if (source / "images").exists() and (source / "labels").exists():
        print("Found standard YOLO dataset structure (images + labels)")
        copy_yolo_dataset(source, target_dir)
    
    # Case 2: Your structure (images + label)
    elif (source / "images").exists() and (source / "label").exists():
        print("Found custom dataset structure (images + label)")
        organize_label_images_dataset(source, target_dir)
    
    # Case 3: Nested standard structure
    ...
    # Case 4: Other custom structure, need to organize
    else:
...
def organize_label_images_dataset(source, target):
    """Organize dataset with separate images and label folders"""
    
    print("\nOrganizing images/label dataset...")
    ...
    # Find all image files in images folder
    ...
    # Find and copy labels from label folder
    ...
    if labels_found == 0:
        print("  Warning: No label files found in 'label' folder")
